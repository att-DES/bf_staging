...
if(accessToken==null || accessToken=="null") {
	accessToken = "";
	session.setAttribute("postOauth", "WAP.jsp");
	%><a href="oauth.jsp">Authenticate first</a><br><br><%
}
String sendWap = request.getParameter("sendWap");
String endpoint = "https://beta-api.att.com/1/messages/outbox/wapPush";		
...
<form name="sendWap" action="WAP.jsp?sendWap=true" enctype="multipart/form-data" method="post">
	...
	<input type="submit" name="sendWap" value="Send WAP Push"/>
</form>
...
<% if(request.getParameter("sendWap")!=null) {	
		...
		if (requestFormat.equalsIgnoreCase("form-encoded"))
		{
			...
			JSONObject requestObject = new JSONObject();
   			...
   			requestObject.put("content-type", "application/xml");
	         ...
	         Element e1 = doc.createElement("wapPushRequest");
	         doc.appendChild(e1);
	         ...
	         e1.appendChild(e5);
	         e5.setTextContent("application/xml");
		...
		if ((client.getHttpResponseCode() == 201) & (responseFormat.equalsIgnoreCase("json"))) {
            ...
            DocumentBuilder builder = factoryForResponse.newDocumentBuilder();
       	    ...
       	    wapId = xmlDoc.getDocumentElement().getAttribute("id");
      	    session.setAttribute("wapId",wapId);
		} else {
			print = responze;
		}
